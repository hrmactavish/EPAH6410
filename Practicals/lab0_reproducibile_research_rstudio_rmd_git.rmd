---
title: "Practical 0 - Reproducible Research in R with the Tidyverse"
...
```{r eval=FALSE}
# I have these installed already
...
The dataset contains 1,846 rows and 3 columns. dataset is which dataset the data are from, x is the x-values, and y are the y values.
...
The Pearson's correlation is -0.064. There is essentially no linear relationship.
...
The Pearson's correlation is -0.063, which is very close to the Pearson's correlation of the dinosaur (-0.064). There is essentially no linear relationship.
...
The Pearson's correlation is -0.068, which is very close to the Pearson's correlation of the dinosaur (-0.064). There is essentially no linear relationship.
...
Although the datasets have very different visual shapes, their summary statistics are nearly identical because the points are distributed around similar centers and spreads. The mean_x, mean_y, standard deviations, and Pearson's correlations are all almost the same across datasets, meaning the overall numerical properties are very similar even though the actual patterns are very visually different. This demonstrates that summary statistics alone can hide important structures, so visualizing data is very important in exploratory data analysis, especially for clinical datasets where clusters, non-linear trends, or outliers may be clinically meaningful.
...
aq %>%
  summarise(
    ...
    sd_temp = sd(Temp),
    complete_days = sum(complete.cases(Ozone, Temp))
  )
...
aq %>%
  ggplot(aes(x = Ozone, y = Temp, colour = month_name)) +
  ...
  labs(
    ...
    colour = "Month",
    title  = "Daily mean ozone versus maximum daily temperature"
  ) +
  theme_minimal()
```
