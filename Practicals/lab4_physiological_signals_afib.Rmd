...
rr_dictionary <- data.frame(
  ...
  Description = c(
    ...
    "Heart rhythm classification (AF or Normal).",
    "Beat number within the 128-beat window (zero-indexed).",
...
We split by record instead of segment because multiple segments from the same patient appear in the dataset, and data from the same patient is correlated; they share the same heart physiology and anatomy, and same recording conditions. If we split by segment, the same patient could appear in both training and test sets, and the model could learn to recognize individual patients rather than AF itself. This would produce overly optimistic performance estimates that would likely not generalize to new patients.
...
A Precision-Recall curve is another option for threshold selection. It could be useful here as there is a class imbalance in the dataset.
...
The best predictors of AF in this dataset are `hf_frac` (p < 0.001), `spectral_centroid` (p < 0.001), and `spectral_entropy` (p < 0.001), which are all highly significant. `peak_frac` contributes little to nothing to the model's predictions (p = 0.681). 

The results mostly match the hypotheses. Both `spectral_entropy` and `spectral_centroid` have positive coefficients, confirming that AF windows have more spread-out spectra and higher centre-of-mass frequencies. `hf_frac` however has a negative coefficient, which is contrary to the hypothesis that AF pushes more power into high frequencies, it potentially suggests that the relationship between high-frequency power and AF may be more complex than was previously hypothesized. `peak_frac` contributes little to the model regardless of the direction of the estimation.
...
As shown in the ROC plot, all three of the models perform fairly poorly on the test set, with none achieving what would typically be considered strong discrimination (AUC $\ge$ 0.80). The time model (ARIMA) model performed the best with an AUC of 0.692, followed by frequency logistic regression model (AUC = 0.625), and the state-space HMM performed the worst (AUC = 0.579, barely above chance). In the ROC plot, this ordering it visible; the blue ARIMA curve bends the closest to the top-left, while the green HMM curve is barely separated from the diagonal. 
