...
The best predictors of AF in this dataset are `spectral_centroid` (p < 0.001), `spectral_entrioy` (p < 0.001), and `hf_frac` (p < 0.001), which are all highly significant. `peak_frac` is not a useful predictor because it is not statistically significant (p = 0.681). This mostly matches the hypotheses; higher entropy and centroid in AF are both confirmed. However, `hf_frac` has a negative coefficient, contradicting the expectation that AF pushes more power into high frequencies, possibly due to collinearity with `spectral_centroid`.
