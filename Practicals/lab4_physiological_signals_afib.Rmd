...
There is a table below with the data dictionary:

```{r}
waveforms_dictionary <- data.frame(
  Dataset = "waveforms",
  Variable = c("record", "segment", "rhythm", "time", "ecg"),
  Format = c(
    "Character",
    "Character",
    "Factor",
  ...
  Description = c(
    ...
    "Identifier for a 10-second ECG segment from a patient record (patient + rhythm).",
    "Heart rhythm classification (AF or Normal).",
    "Time point within the ECG segment.",
    "ECG voltage measurement recorded at each time point."
  )
)

rr_dictionary <- data.frame(
  Dataset = "rr",
  Variable = c("record", "window", "rhythm", "beat", "rr"),
  Format = c(
    "Character",
    "Numeric",
    "Factor",
  ...
  Description = c(
    "Unique identifier for each patient record.",
    "Identifier for which 128-beat window the beat belongs to (zero-indexed).",
    "Heart rhythm classification (AF or Normal).",
    "Beat number within the 128-beat window.",
    "R-R interval length between consecutive heartbeats."
  )
)
...
knitr::kable(
  data_dictionary,
  caption = "Combined Data Dictionary"
)
```
...
We split by record instead of segment so that the data from the same patient does not appear in both the training and test sets. Segments from the same patient are correlated, they share the same heart, physiology, and recording conditions. If we split by segment, the models could learn patient-specific ECG characteristics instead of learning features associated with AF. This would lead to overly optimistic performance estimates and failure when applied to external validation.
...
fs/2 is the Nyquist frequency, which is the maximum frequency that can be captured in a signal sampled at fs Hz. Because the ECG was sampled at 250 Hz, fs/2 = 125 Hz, meaning that frequencies above 125 Hz cannot be reliably captured.
...
The plot shows that AF windows have shorter mean R-R intervals than Normal windows, suggesting a faster average heart rate. AF windows also have a higher SDNN values, indicating that there is more heart rate variability, which is expected given that AF is a type of cardiac arrhythmia.

Overall, AF rhythms are more irregular and variable, whereas Normal rhythms have longer and more consistent R-R intervals.
...
A Precision-Recall curve is another option for threshold selection. Given that there is a class imbalance, where AF is the majority class, a precision-recall curve would be useful because it would also highlight how well the model performs on the minority class (Normal).
...
The best predictors of AF in this dataset are `spectral_centroid` (p < 0.001), `spectral_entropy` (p < 0.001), and `hf_frac` (p < 0.001), which are all highly significant. `peak_frac` contributes little to nothing to the model's predictions (p = 0.681). 

The results mostly match the hypotheses. Both `spectral_entropy` and `spectral_centroid` have positive coefficients, confirming that AF windows have more spread-out spectra and higher centre-of-mass frequencies. `hf_frac` however has a negative coefficient, which is contrary to the hypothesis that AF pushes more power into high frequencies, it potentially suggests taht the relationship between high-frequency power and AF may be more complex than was previously hypothesized. `peak_frac` contributes little to the mode regardless of the direction of the estimation.
...
It means that the model is performing worse than random guessing at those threshold values. It is more likely to misclassify observations than classify them correctly. This sometimes occurs when the score is accidentally inverted or the event level is not correctly specified (e.g. the event is coded as 0 when it should be 1), but this is not the case here, as only a small portion of the curve dips below the line.
...
In this HMM, the observed variables are the R-R interval measurements. The hidden variables are the two underlying rhythm states, which are never directly measured but are learned by the model from the distribution of R-R intervals. The two states correspond to a "regular" (Normal) and a more "irregular" (AF) state.
...
As shown in the ROC plot, all three of the models perform fairly poorly on the test set, with non achieving what would typically be considered strong discrimination (AUC $\ge$ 0.80). The time model (ARIMA) model performed the best with an AUC of 0.692, followed by frequency logistic regression model (AUC = 0.625), and the state-space HMM performed the worst (AUC = 0.579, barely above chance). In the ROC plot, this ordering it visible; the blue ARIMA curve bends the closest to the top-left, while the green HMM curve is barely separated from the diagonal. 

This suggests that the predictability of R-R intervals was the most useful feature for detecting AF in this dataset, which makes sense given that unpredictability of the rhythm is a defining feature of AF, while the frequency and state-space approaches were less effective.
...
I was unsure about if this question was asking for one improvement across all three models or one per model, so I tried to address both below:

One improvement applicable to all three models would be to use longer windows of R-R interval data. The waveform snippets that we used are only 10 seconds (29 beats), which may not capture enough of the irregular pattern of AF to reliably distinguish it from normal rhythm. Using the full 128-beat (~ 2 minute) windows from the rr dataset, or longer recordings, would give each model more signal to work with. 

If one improvement per model is intended: the ARIMA model could be improved by training on more data, as the current training set is relatively small and a larger sample of patients would help the model better learn what typical Normal and AF intervals look like. The frequency model currently uses logistic regression to combine the four features into an AF probability, but a more flexible classifier such as a random forest could better capture non-linear relationships that might exist between the features and AF. The HMM could be extended to more than two hidden states to better capture intermediate or transitional rhythms rather than forcing a binary regular or irregular distinction.

Before trying any of these models in the real world, at least three additional analyses should be preformed:

1. Subgroup performance analysis: model performance should be evaluated across clinically relevant subgroups such as age, sex, and AF type to identify whether the models perform systematically worse for certain patient groups.
2. Specificity to AF vs other arrhythmias: the models should be tested against patients with other irregular rhythms, such as ventricular tachycardia, to determine whether they are detecting AF specifically or simply any deviation from regular sinus rhythm.
3. External validation: all three models should be tested on an entirely independent dataset from a different institution or recording device to confirm that the performance generalizes outside of the MIT-BIH Atrial Fibrillation Database used in this study.
